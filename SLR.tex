...
%   LDB (zhong2024ldb) | Self-Edit (zhang2023selfedit) |
...
\midrule
4 & Verification \& Test Generation & \tbd & Liu et al.~\cite{liu2025llm4tdg} (LLM4TDG), Chen et al.~\cite{chenbei2023} (CodeT), Ni et al.~\cite{ni2023lever} (LEVER), Ma et al.~\cite{ma2025saga} (SAGA), Sun et al.~\cite{sun2024clover} (Clover), \tbd\ (e.g., EvalPlus~\cite{liu2023evalplus} if included) \\
\midrule
...
\midrule
1 & Zero-shot prompting & \tbd & \cite{chen2021humaneval,xia2023apr,wang2024intervenor,shinn2023,sun2024clover,chenbei2023}, \tbd \\
...
4 & LLM-generated test verification & \tbd & \makecell{N/A for CodeT (post-hoc selection; no repair);\\Reflexion trial cap not stated\\(episodic memory: 1 experience for programming)} & \cite{liu2025llm4tdg,shinn2023,ma2025saga,chenbei2023} \\
...
CodeT~\cite{chenbei2023} & Code available; license not reported & Not reported & code-cushman-001, code-davinci-001/002, InCoder-6B, CodeGen-Mono-16B & Python & LLM-generated input--output tests + candidate execution + dual execution agreement over consensus sets & No ground-truth tests used for selection; hidden benchmark tests used only for final pass@k evaluation & Not reported \\
...
MBPP/MBPP+~\cite{austin2021mbpp,chenbei2023} & 974 / 399 & Python & Crowd-sourced / augmented tests & Correctness & Moderate \\
...
\textbf{Answer:} As shown in Table~\ref{tab:repair_approaches}, \tbd\ is the most frequently represented mechanism (\tbd\ studies, \tbd\%). Regarding iteration budgets, \tbd\ of the repair-capable studies employ a fixed small budget ($\leq$3 iterations), \tbd\ employ larger or unbounded schedules, and \tbd\ operate post-hoc without iteration. CodeT belongs to the last group: it performs one post-hoc verification-and-ranking pass over independently sampled programs and generated tests, without modifying or repairing any candidate~\cite{chenbei2023}. The only taxonomy-grounded fixed-budget datapoint in the reviewed literature is the two-iteration pipeline of Dou et al.~\cite{dou2026}, which achieves a 29.2\% repair success rate on GPT-4. Bug-taxonomy-guided repair, constraint-based reasoning (CDG/CDCL), and runtime-trace debugging remain represented by single studies each, indicating specialized but potentially underexplored directions. These results indicate a diversified verification-and-repair landscape in which the iteration budget is rarely treated as a first-class design variable.
...
\bibitem{chenbei2023}
B. Chen, F. Zhang, A. Nguyen, D. Zan, Z. Lin, J.-G. Lou, and W. Chen, ``CodeT: Code generation with generated tests,'' in \textit{The Eleventh International Conference on Learning Representations (ICLR)}, 2023.
