...
%  3) The primary-study set is frozen at 37 distinct studies.
%     Citation details still requiring checks are flagged individually.
...
% category row
\node[cat] (c1) at (-6.1,-3.3) {General\\ 2 papers};
\node[cat] (c2) at (-3.65,-3.3) {Code\\Generation\\ 9 papers};
\node[cat] (c3) at (-1.2,-3.3) {Bug Detection\\\& Classification\\ 2 papers};
\node[cat] (c4) at (1.25,-3.3) {Verification \&\\Test Generation\\ 8 papers};
...
\node[outbox] (o2) at (0,-6.6)    {Preliminary Tools\\Identification: 32};
...
% tool categories row
\node[tool] (t1) at (-5.9,-9.8) {Generation\\Approaches\\ 4};
\node[tool] (t2) at (-2.95,-9.8) {Verification\\Tools\\ 7};
\node[tool] (t3) at (0,-9.8)    {Repair\\Tools\\ 11};
\node[tool] (t4) at (2.95,-9.8) {Optimization\\Tools\\ 3};
\node[tool] (t5) at (5.9,-9.8)  {Security\\Tools\\ 4};
% arrows
...
\item The objective is to include the most recent researches as much as possible. Therefore, 86.5\% of the selected researches were published from 2023 to 2026, as shown in Fig.~\ref{fig:year_distribution}.
...
% Bar values reflect the final frozen set of 37 primary studies.
\begin{figure*}[!t]
\centering
\begin{tikzpicture}
\begin{axis}[
...
\addplot[fill=headerblue!70,draw=headerblue] coordinates {
    (2021,3) (2022,2) (2023,10) (2024,11) (2025,9) (2026,2)
};
\end{axis}
\end{tikzpicture}
...
\section{Results}
\label{sec:results}
37 primary studies were identified and classified across the six categories. The results are presented through structured synthesis tables, following the methodological guidelines that favor quantitative classification over paragraph-based summaries.

\subsection{Classification of Selected Researches}
\label{sec:classification}
Table~\ref{tab:classification} summarizes the classification of the 37 selected researches into the six predefined categories.

\begin{table*}[!t]
\centering
\caption{Classification of 37 selected researches across the six predefined categories.}
...
\midrule
1 & General & 2 & Dou et al.~\cite{dou2026}, Ding et al.~\cite{ding2025codingcare} (CodingCare) \\
\midrule
2 & Code Generation & 9 & Li et al.~\cite{li2025scot} (SCoT), Jiang et al.~\cite{jiang2024selfplan} (Self-Planning), Chen et al.~\cite{chen2021humaneval} (HumanEval), Austin et al.~\cite{austin2021mbpp} (MBPP), Hendrycks et al.~\cite{hendrycks2021apps} (APPS), Zhang et al.~\cite{zhang2025abfcg} (ABFCG), Jimenez et al.~\cite{jimenez2024swebench} (SWE-bench), Le et al.~\cite{le2022coderl} (CodeRL), Jain et al.~\cite{jain2024} (LiveCodeBench) \\
\midrule
3 & Bug Detection \& Classification & 2 & Dou et al.~\cite{dou2026}*, Tambon et al.~\cite{tambon2025} \\
\midrule
4 & Verification \& Test Generation & 8 & Liu et al.~\cite{liu2025llm4tdg} (LLM4TDG), Chen et al.~\cite{chenbei2023} (CodeT), Ni et al.~\cite{ni2023lever} (LEVER), Ma et al.~\cite{ma2025saga} (SAGA), Sun et al.~\cite{sun2024clover} (Clover), Zhong et al.~\cite{zhong2024ldb} (LDB)*, Liu et al.~\cite{liu2023evalplus} (EvalPlus), Zhang et al.~\cite{zhangt2023coderreviewer} (Coder-Reviewer reranking) \\
\midrule
...
\midrule
\multicolumn{2}{r}{\textbf{Total category assignments:}} & 40 & \\
\multicolumn{2}{r}{\textbf{Total distinct studies:}} & 37 & \\
\bottomrule
\end{tabular}
\begin{flushleft}
...
% TODO: recount rows against the 37-study frozen list in Step 2
\begin{table*}[!t]
\centering
...
2 & Few-shot prompting & \tbd & \cite{austin2021mbpp,hendrycks2021apps,li2025scot,jiang2024selfplan,chen2023selfdebug,xia2023apr,madaan2023,xia2024chatrepair}, \tbd \\
...
\midrule
1 & Self-critique with compiler/execution feedback & \tbd & \makecell[l]{Varied: 2 (Dou et al.); 3 conversation turns\\(ChatRepair default); 1--2 (Self-Edit);\\unbounded/varied (Olausson et al.)} & \cite{dou2026,chen2023selfdebug,wang2024intervenor,olausson2024,zhang2023selfedit,xia2024chatrepair}, \tbd\ (CYCLE) \\
...
4 & LLM-generated test verification & \tbd & \makecell{CodeT: N/A (post-hoc, no iteration);\\Reflexion trial cap not stated\\(episodic memory: 1 experience for programming)} & \cite{liu2025llm4tdg,shinn2023,ma2025saga,chenbei2023} \\
...
9 & Massive-sample generation with entropy/test-based ranking & \tbd & N/A (post-hoc) & \cite{xia2023apr,lyu2025toppass,chenbei2023}, \tbd \\
...
\midrule
\multicolumn{3}{l}{\textit{Code Generation via RL (1)}} \\
\midrule
4 & CodeRL~\cite{le2022coderl} & \cite{le2022coderl} \\
\midrule
\multicolumn{3}{l}{\textit{Verification \& Test Generation Tools (7)}} \\
\midrule
...
\midrule
\multicolumn{3}{l}{\textit{Iterative Self-Repair Tools (11)}} \\
\midrule
12 & Self-Debug~\cite{chen2023selfdebug} & \cite{chen2023selfdebug,dou2026} \\
13 & INTERVENOR~\cite{wang2024intervenor} & \cite{wang2024intervenor,dou2026} \\
14 & Dou et al.\ repair pipeline~\cite{dou2026} & \cite{dou2026} \\
15 & Xia et al.~\cite{xia2023apr} & \cite{xia2023apr} \\
16 & Self-Refine~\cite{madaan2023} \tbd & \tbd \\
17 & Reflexion~\cite{shinn2023} & \cite{shinn2023} \\
18 & Self-Edit~\cite{zhang2023selfedit} & \cite{zhang2023selfedit} \\
19 & ChatRepair~\cite{xia2024chatrepair} & \cite{xia2024chatrepair} \\
20 & CYCLE~\cite{ding2024cycle} & \cite{ding2024cycle} \\
21 & Olausson et al.\ self-repair study~\cite{olausson2024} & \cite{olausson2024} \\
22 & LDB~\cite{zhong2024ldb} & \cite{zhong2024ldb} \\
\midrule
\multicolumn{3}{l}{\textit{Static Analysis Tools (3)}} \\
\midrule
23 & CodeQL & \cite{bruni2025,ding2025codingcare} \\
24 & Semgrep & \cite{bruni2025,zhao2025semopt} \\
25 & Bandit & \cite{ding2025codingcare,nazzal2024promsec} \\
\midrule
\multicolumn{3}{l}{\textit{Optimization \& Security Tools (7)}} \\
\midrule
26 & SemOpt~\cite{zhao2025semopt} & \cite{zhao2025semopt} \\
27 & Top Pass~\cite{lyu2025toppass} & \cite{lyu2025toppass} \\
28 & Copilot-in-the-Loop~\cite{zhang2024copilot} & \cite{zhang2024copilot} \\
29 & PromSec~\cite{nazzal2024promsec} & \cite{nazzal2024promsec} \\
30 & CodingCare~\cite{ding2025codingcare} & \cite{ding2025codingcare} \\
31 & SVEN~\cite{he2023sven} & \cite{he2023sven} \\
32 & Pearce et al.~\cite{pearce2022} & \cite{pearce2022} \\
\bottomrule
\end{tabular}
\end{table*}
...
CodeRL~\cite{le2022coderl} & [VERIFY] & [VERIFY] & [VERIFY] & [VERIFY] & RL fine-tuning + critic-based repair & [VERIFY] & [VERIFY] \\
\bottomrule
\end{tabularx}
\end{table*}
...
EvalPlus~\cite{liu2023evalplus} & [VERIFY] & [VERIFY] & [VERIFY] & [VERIFY] & Augmented test suites & Benchmark-side & [VERIFY] \\
Coder-Reviewer reranking~\cite{zhangt2023coderreviewer} & [VERIFY] & [VERIFY] & [VERIFY] & [VERIFY] & Coder-reviewer reranking & [VERIFY] & [VERIFY] \\
...
LDB~\cite{zhong2024ldb} & Code available (GitHub: FloridSleeves/LLMDebugger) & Full & GPT-3.5, CodeLlama-34B, StarCoder-15B (+GPT-4, GPT-4o backbone ablation) & Python; C++$\to$Python (TransCoder) & Runtime traces (block-level) & Partial (visible test needed to trigger debugging; TransCoder uses all tests as visible, no held-out oracle there) & $\sim$23K tokens/program; 17.23s vs.\ 17.08s for Self-Debugging \\
\bottomrule
\end{tabularx}
\end{table*}
...
CYCLE~\cite{ding2024cycle} & [VERIFY] & [VERIFY] & [VERIFY] & [VERIFY] & Execution feedback (trained) & Yes & [VERIFY] & No \\
...
SVEN~\cite{he2023sven} & [VERIFY] & [VERIFY] & [VERIFY] & [VERIFY] & [VERIFY] & [VERIFY] & Training-time (prefix tuning) \\
Pearce et al.~\cite{pearce2022} & [VERIFY] & [VERIFY] & [VERIFY] & [VERIFY] & [VERIFY] & [VERIFY] & [VERIFY] \\
\bottomrule
\end{tabularx}
\begin{flushleft}
...
LiveCodeBench~\cite{jain2024} & [VERIFY] & [VERIFY] & [VERIFY] & Contamination-free correctness & Very Low \\
...
\textbf{Answer:} 37 important researches selected from the 2021--2025 search window (including two protocol exceptions formally published in 2026) have been identified as per the selection and rejection criteria (Section~\ref{sec:criteria}). These researches are classified into six corresponding categories. The details are as follows:
\begin{itemize}[leftmargin=*]
\item 2 researches have been identified in the \textit{General} category.
\item 9 researches have been identified in the \textit{Code Generation} category, with three being foundational benchmark papers (HumanEval, MBPP, APPS).
\item 2 researches have been identified in the \textit{Bug Detection \& Classification} category, anchored by the fine-grained three-tier/ten-category taxonomy of Dou et al.~\cite{dou2026}.
\item 8 researches have been identified in the \textit{Verification \& Test Generation} category.
...
\textbf{Answer:} On the basis of the SLR, 32 tools/frameworks and 28 benchmarks have been identified to support LLM-based code generation across the five activity dimensions. After the comparative evaluation in Section~\ref{sec:comparison}, the following key tools are presented by activity:
\begin{itemize}[leftmargin=*]
\item 4 \textbf{generation approaches} are presented to support structured prompting, planning, and retrieval augmentation (SCoT, Self-Planning, ABFCG, \tbd).
\item 7 \textbf{verification and test generation tools} are presented (LLM4TDG, CodeT, \tbd). CodeT is a post-hoc selector rather than a repair tool: it generates tests, executes sampled candidates, and ranks consensus sets through dual execution agreement~\cite{chenbei2023}. The oracle assumption column of Table~\ref{tab:cmp_verification} is decisive for practical deployment: tools assuming hidden ground-truth tests at inference time cannot be transferred directly to production settings.
\item 11 \textbf{iterative self-repair tools} are presented (Self-Debug, INTERVENOR, Dou et al., Xia et al., \tbd). Among these, LLM4TDG notably achieves the highest \texttt{pass@k} improvement (18.66\% on CodeQwen-chat) and validates its approach on real malicious packages.
\item 3 \textbf{optimization tools} are presented (SemOpt, Jonnala et al., Top Pass, Copilot-in-the-Loop, \tbd).
\item 4 \textbf{security tools} are presented (PromSec, CodingCare, Bruni et al., \tbd).
...
\item 28 \textbf{benchmarks} span correctness and translation, repair, data-science programming, efficiency and optimization, maintainability, security, competitive programming, and real-world leakage-free evaluation (RWPB, SWE-bench~\cite{jimenez2024swebench}, LeetcodeHardGym~\cite{shinn2023}, \tbd).
\end{itemize}
...
\end{itemize}
Such a framework would represent the first unified single-agent response to the verification-and-repair demands identified across the 37 reviewed studies, targeting repair success beyond the 29.2\% two-iteration baseline at a fraction of the token cost documented for multi-agent alternatives.
...
This Systematic Literature Review has rigorously identified, classified, and analyzed 37 primary studies on LLM-based code generation selected from the 2021--2025 search window, including two studies retained under the search protocol and formally published in 2026. Following a structured review protocol encompassing precise research questions, six concrete selection criteria, a multi-stage filtering process yielding 37 primary studies from an initial pool of \tbd\ candidates, and a comprehensive data extraction framework, the review provides an evidence-based synthesis of contemporary techniques, tools, and benchmarks.
...
% -------------------- PRIMARY STUDIES (frozen set of 37) --------------------
\bibitem{dou2026}
...
\bibitem{ridnik2024}
T. Ridnik, D. Kredo, and I. Friedman, ``Code generation with AlphaCodium: From prompt engineering to flow engineering,'' \textit{arXiv preprint arXiv:2401.08500}, 2024. % CITATION-UNCONFIRMED: verify exact pages/venue before camera-ready

\bibitem{liu2023evalplus}
J. Liu, C. S. Xia, Y. Wang, and L. Zhang, ``Is your code generated by ChatGPT really correct? Rigorous evaluation of large language models for code generation,'' in \textit{Advances in Neural Information Processing Systems (NeurIPS)}, vol. 36, 2023. % CITATION-UNCONFIRMED: verify exact pages/venue before camera-ready

\bibitem{xia2024chatrepair}
C. S. Xia and L. Zhang, ``Automated program repair via conversation: Fixing 162 out of 337 bugs for \$0.42 each using ChatGPT,'' in \textit{Proceedings of the 33rd ACM SIGSOFT International Symposium on Software Testing and Analysis (ISSTA)}, 2024. % CITATION-UNCONFIRMED: verify exact pages/venue before camera-ready

\bibitem{ding2024cycle}
Y. Ding, M. J. Min, G. Kaiser, and B. Ray, ``CYCLE: Learning to self-refine the code generation,'' \textit{Proceedings of the ACM on Programming Languages (OOPSLA)}, 2024. % CITATION-UNCONFIRMED: verify exact pages/venue before camera-ready
...
T. Zhang, T. Yu, T. Hashimoto, M. Lewis, W. Yih, D. Fried, and S. I. Wang, ``Coder reviewer reranking for code generation,'' in \textit{Proceedings of the 40th International Conference on Machine Learning (ICML)}, 2023. % CITATION-UNCONFIRMED: verify exact pages/venue before camera-ready

\bibitem{le2022coderl}
H. Le, Y. Wang, A. D. Gotmare, S. Savarese, and S. C. H. Hoi, ``CodeRL: Mastering code generation through pretrained models and deep reinforcement learning,'' in \textit{Advances in Neural Information Processing Systems (NeurIPS)}, vol. 35, 2022. % CITATION-UNCONFIRMED: verify exact pages/venue before camera-ready
...
\bibitem{pearce2022}
H. Pearce, B. Ahmad, B. Tan, B. Dolan-Gavitt, and R. Karri, ``Asleep at the keyboard? Assessing the security of GitHub Copilot's code contributions,'' in \textit{Proceedings of the 2022 IEEE Symposium on Security and Privacy (S\&P)}, 2022, pp. 754--768. % CITATION-UNCONFIRMED: verify exact pages/venue before camera-ready

\bibitem{he2023sven}
J. He and M. Vechev, ``Large language models for code: Security hardening and adversarial testing,'' in \textit{Proceedings of the 2023 ACM SIGSAC Conference on Computer and Communications Security (CCS)}, 2023. % CITATION-UNCONFIRMED: verify exact pages/venue before camera-ready

\bibitem{jain2024}
N. Jain, K. Han, A. Gu, W.-D. Li, F. Yan, T. Zhang, S. Wang, A. Solar-Lezama, K. Sen, and I. Stoica, ``LiveCodeBench: Holistic and contamination free evaluation of large language models for code,'' \textit{arXiv preprint arXiv:2403.07974}, 2024. % CITATION-UNCONFIRMED: verify exact pages/venue before camera-ready
